		...
		try {
			...
			if (apkCode > versionCode) {
				if (mTypeOfNotice == NOTICE_NOTIFICATION) {
					...
					Log.i("成功",apkUrl);
					Toast.makeText(mContext, mContext.getString(R.string.download_progress), Toast.LENGTH_SHORT).show();
				}
			} else {
...
//		RxPermissions.getInstance(getContext()).request(Manifest.permission.WRITE_EXTERNAL_STORAGE).subscribe(granted -> {
//			if (granted) {
//				Intent myIntent = new Intent(getContext(), DownloadService.class);
