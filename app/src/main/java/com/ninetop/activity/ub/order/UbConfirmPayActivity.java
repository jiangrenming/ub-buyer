...
public class UbConfirmPayActivity extends BaseActivity {
  ...
  /*  private void select(int index) {
    ...
    private void getDetail() {
        ...
        orderCode = bean.orderCode;
        ubPay = bean.ubPay;
        payPrice = bean.moneyPay;
        tvBalancePay.setText(bean.balancePay+"");
        if (payPrice > 0) {
        } else if (payPrice == 0) {
            payType = 1;
        }
        long myAll = (long) Math.floor(bean.totalPay);
        long myUb = (long) Math.floor(ubPay);
        tvOrderNum.setText(orderCode);
     //   tvTotalPay.setText(myAll + "");
        tvUPay.setText(bean.getTotalProductPay() + "");
    }
